...
// Создание объектов
const imgPopup = new PopupWithImage(selectors.imgPopupSelector);
imgPopup.setEventListeners();

const section = new Section({
  items: initialCards,
  renderer: (item) => {
    const card = new Card(item, selectors.cardTemplateSelector, imgPopup.open);
...
const profilePopup = new PopupWithForm(selectors.profilePopupSelector, function (inputValues) {
  userInfo.setUserInfo(inputValues);
  this.close();
});
profilePopup.setEventListeners();

const cardPopup = new PopupWithForm(selectors.cardPopupSelector, (inputValues) => {
  const card = new Card(inputValues, selectors.cardTemplateSelector, imgPopup.open);
  const cardElement = card.getCardElement();
